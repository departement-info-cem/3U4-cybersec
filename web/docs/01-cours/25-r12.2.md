...
**Outil recommandé**: https://www.toolhelper.cn/en/SymmetricEncryption/Blowfish

**Paramètres à configurer**:
- Algorithm: L'algorithme trouvé dans le code
- Key: La clé trouvée dans le code
- IV: Le vecteur d'initialisation trouvé
- Mode: Le mode de chiffrement (CTR, CBC, etc.)
- Padding: Le type de padding utilisé
- Input format: Base64 (les données sont encodées en Base64)

:::warning Attention aux formats
- La clé peut être en texte ou en hexadécimal
- L'IV est souvent en bytes, il faudra peut-être le convertir en hex
